...
public class DispNameCE implements CommandExecutor {
     ...
     * @param args   the args given with the command.
     *
    ...
    public boolean check(CommandSender sender, String[] args) {
        ...
        // Name Given to check for.
            ...
            if (players.length == 0) {
                api.sendMessage(DNCStrings.ERROR_BAD_USER, sender, null, MessageType.ERROR);
                ...
                api.sendMessage(DNCStrings.INFO_CHECK_SINGLE, sender, users, MessageType.INFO);
                ...
                api.sendMessage(DNCStrings.INFO_CHECK_MULTI, sender, null, MessageType.CONFIRMATION);

                api.sendMessage(DNCStrings.INFO_CHECK_MULTI_LIST, sender, users, MessageType.INFO);

                return true;
            }
        }
        else if (saArgs.length > 1) {
            api.sendMessage(DNCStrings.ERROR_BAD_ARGS, sender, null, MessageType.ERROR);
        }
    ...
    public boolean list(CommandSender sender, String[] args) {
        ...
        if (plugin.getPagination() == 0) {
            ...
            for (Player p : players) {
                if (!p.getName().equals(p.getDisplayName())) {
                    ...
                    api.sendMessage(DNCStrings.INFO_CHECK_SINGLE, sender, oNames, MessageType.INFO);
                }
            }
        ...
        if (args.length == 0) {
            ...
            try {
                ...
                api.sendMessage(DNCStrings.ERROR_BAD_INPUT, sender, null, MessageType.ERROR);
        ...
        if (iPageNumber >= iMaxPages) {
            api.sendMessage(DNCStrings.INFO_LIST_MAX, sender, oArgs, MessageType.CONFIRMATION);
        }
        else {
            api.sendMessage(DNCStrings.INFO_LIST, sender, oArgs, MessageType.CONFIRMATION);
        }
        ...
        switch (saArgs.length) {
            ...
            case 1:
                ...
                if (api.isConsole(sender)) {
                    api.sendMessage(DNCStrings.ERROR_CONSOLE_RENAME, sender, null, MessageType.ERROR);

                    return true;
                }

                // Check for colors we aren't allowed to use.
                ChatColor badColor = api.checkForIllegalColors((Player) sender, saArgs[0]);

                if (badColor != null) {
                    Object[] obj = {badColor};

                    if (badColor.isColor()) {
                        api.sendMessage(DNCStrings.PERMISSION_COLOR, sender, obj, MessageType.ERROR);
                    }
                    else {
                        api.sendMessage(DNCStrings.PERMISSION_STYLE, sender, obj, MessageType.ERROR);
                    }
                ...
                if (saArgs[0].contains(" ")) {
                    if (!api.canUseChangeNameSpace(sender)) {
                        api.sendMessage(DNCStrings.PERMISSION_SPACES, sender, null, MessageType.ERROR);
                ...
                try {
                    ...
                    api.sendMessage(DNCStrings.ERROR_NON_UNIQUE, sender, null, MessageType.ERROR);
            ...
            case 2:
                ...
                // Target Match Found
                    ...
                    if ((sender instanceof Player) && !players[0].equals(sender)) {
                    ...
                    api.sendMessage(DNCStrings.ERROR_MULTI_MATCH, sender, null, MessageType.ERROR);
        ...
        if (saArgs.length == 0) {
            if (api.isConsole(sender)) {
                ...
                try {
                    ...
                    log.severe("There was an error reverting from the DisplayName: Display - " + p.getDisplayName() + " | BadNick: " + e.getBadName());
            ...
            switch (players.length) {
                ...
                case 1:
                    ...
                    try {
                        ...
                        log.severe("There was an error reverting from the DisplayName: Display - " + sDisplayName + " | BadNick: " + e.getBadName());
     ...
     *
     * @param size number of entries total.
     *
