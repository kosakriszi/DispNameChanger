...
public class DispNameAPI {
    ...
    public boolean canUseChangeNameOther(CommandSender s) {
        return s == null || s.hasPermission("dispname.changeother");
    }
    ...
    public boolean canUseChangeNameSpace(CommandSender s) {
        return s == null || s.hasPermission("dispname.changespace");
    }
    ...
    public boolean canUseCheckName(CommandSender s) {
        return s == null || s.hasPermission("dispname.check");
    }
    ...
    public boolean canUseColorAqua(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.aqua");
    }
    ...
    public boolean canUseColorBlack(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.black");
    }
    ...
    public boolean canUseColorBrightGreen(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.brightgreen");
    }
    ...
    public boolean canUseColorDarkAqua(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.darkaqua");
    }
    ...
    public boolean canUseColorDarkBlue(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.darkblue");
    }
    ...
    public boolean canUseColorDarkGreen(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.darkgreen");
    }

    public boolean canUseColorDarkGrey(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.darkgrey");
    }
    ...
    public boolean canUseColorDarkRed(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.darkred");
    }
    ...
    public boolean canUseColorGrey(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.grey");
    }
    ...
    public boolean canUseColorIndigo(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.indigo");
    }
    ...
    public boolean canUseColorOrange(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.orange");
    }
    ...
    public boolean canUseColorPink(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.pink");
    }
    ...
    public boolean canUseColorPurple(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.purple");
    }
    ...
    public boolean canUseColorRed(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.red");
    }
    ...
    public boolean canUseColorWhite(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.white");
    }
    ...
    public boolean canUseColorYellow(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.yellow");
    }

    /**
     * Checks to see if the Sender can use the list command.
     *
     * @param s the player to check the permission for.
     *
     * @return true if the player can use the command, false otherwise.
     */
    public boolean canUseList(CommandSender s) {
        return s == null || s.hasPermission("dispname.list");
    }
    ...
    public boolean canUseStyleBold(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.style.bold");
    }
    ...
    public boolean canUseStyleItalic(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.style.italic");
    }
    ...
    public boolean canUseStyleMagic(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.style.magic");
    }
    ...
    public boolean canUseStyleStrikethrough(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.style.strike");
    }
    ...
    public boolean canUseStyleUnderline(CommandSender s) {
        return s == null || s.hasPermission("dispname.color.style.underline");
    }
     ...
     *
     * @throws NonUniqueNickException if Non-Unique name is specified and Scoreboard is
     *                                Enabled.
     */
    public void changeDisplayName(Player target, String newName) throws NonUniqueNickException {
     ...
     *
     * @param sender  The person trying to make the change.
     * @param target  The intended target of the change.
     * @param newName The new name to use.
     *
     * @throws NonUniqueNickException if Non-Unique name is specified and Scoreboard
     *                                integration is enabled.
     */
    public void changeDisplayName(CommandSender sender, Player target, String newName) throws NonUniqueNickException {
        Object[] users = new Object[2];

        users[0] = (target.getDisplayName() + ChatColor.GREEN);

        // Parse Color Codes.
        newName = parseColors(newName);

        // Attach the prefix if needed.
        if (!target.getName().equals(newName)) {
            newName = prefixNick(newName);
        }

        if (plugin.useScoreboard()) {
            // Ensure unique
            if (!checkUnique(newName)) {
                throw new NonUniqueNickException(newName);
            }
        }

        users[1] = (newName + ChatColor.GREEN);
        ...
        if (plugin.useScoreboard()) {
            ...
            if (newName.length() > 16) {
                sListName = newName.substring(0, 16);
        ...
        sendMessage(DNCStrings.INFO_NICK_TARGET, target, users, MessageType.CONFIRMATION);

        if (sender != null && !sender.equals(target)) {
            sendMessage(DNCStrings.INFO_NICK_CALLER, sender, users, MessageType.CONFIRMATION);
        }
            ...
            if (targets.length > 0) {
                for (Player p : targets) {
                    if (plugin.isBroadcastAll()) {
                        sendMessage(DNCStrings.INFO_NICK_CALLER, p, users, MessageType.INFO);
                    }
                    else {
                        if (canBroadcast(p)) {
                            sendMessage(DNCStrings.INFO_NICK_CALLER, p, users, MessageType.INFO);
     ...
     *
     * @param nick The nick to check for colors.
     *
     ...
     * without permission).
     *
     * @param target Player who called the name change.
    ...
    public Player[] checkForNames(String text) {
        ...
        ArrayList<Player> pFound = new ArrayList<>();
    ...
    public ChatColor[] getColorsUsed(String input) {
        ArrayList<ChatColor> al = new ArrayList<>();
        ChatColor[] colors;
     ...
     *
     * @param exclude Array of players to exclude from resulting array.
     *
    ...
    public boolean isLastColorReset(String input) {
        ...
        return ccColor != null && ccColor == ChatColor.RESET;
    }

    /**
     * Returns the last color used in a string.
     *
     * @param input The nick to check
     *
        ...
        // Check to see if target/name given.
            ...
            if (args[0].startsWith("\"")) {
                if (!args[0].endsWith("\"")) {
                    ...
                    return new String[] {sTarget, sName};
                }
            }
            // <target> + <name> given.
            else {
     ...
     * name.
     *
     * @param name the name to parse colors for.
     *
     ...
     *
     * @param player the player to restore the name from.
     */
    public void restoreNick(Player player) {
        ...
        if (pClass == null) {
            ...
            pClass.setPlayerID(player.getUniqueId().toString());
        ...
        if (plugin.useScoreboard()) {
            for (Player p : plugin.getServer().getOnlinePlayers()) {
                ...
                if (p.getDisplayName().equals(sDName)) {
                    ...
                    player.sendMessage(ChatColor.RED + DNCStrings.dnc_short + plugin.getLocalization().getString(DNCStrings.INFO_NICK_CONFLICT));
     ...
     * @param args    Any arguments that the formatter will need to embed in
     *                the string.
     * @param type    The type of message to send, specifically how it will be
     *                colored.
     */
    public void sendMessage(DNCStrings message, CommandSender target, Object[] args, MessageType type) {
        StringBuilder sb = new StringBuilder();

        // Colorize the String using the Plugin and Message Colors
        sb.append(ChatColor.GOLD).append(DNCStrings.dnc_short).append(type.getColor());
     ...
     *
     * @param input string to check for color codes.
     *
     * @return true if one of the special color codes is found, false
     * otherwise.
     */
    public boolean specialChatColor(String input) {
        return input.contains(ChatColor.BOLD.toString()) || input.contains(ChatColor.ITALIC.toString()) || input.contains(ChatColor.STRIKETHROUGH.toString()) || input.contains(ChatColor.UNDERLINE.toString());
    }
     ...
     * is reset.
     *
     * @param player The player to store the display name for.
     */
    ...
    public String stripPrefix(String nick) {
        ...
        if (plugin.usePrefix() && plugin.usePrefixColor()) {
            ...
            else if (sNick.startsWith(plugin.getPrefixColor() + Character.toString(plugin.getPrefix()))) {
                sNick = sNick.substring((plugin.getPrefixColor() + Character.toString(plugin.getPrefix())).length());
            }
            else {
                if (sNick.contains(prefix)) {
                    String[] split = Pattern.compile(Pattern.quote(prefix)).split(sNick, 2);
