...
public class DPL implements Listener {
     ...
     *
     * @param event the event triggered by the death.
     */
    ...
    public void onPlayerJoin(final PlayerJoinEvent event) {
        ...
        if (plugin.changeLogin()) {
            if (api.isNameChanged(player)) {
                event.setJoinMessage(event.getJoinMessage().replaceFirst(player.getName(), player.getDisplayName()));
            }
        }

        //        if (plugin.isSpoutEnabled() && plugin.useSpoutTitle()) {
        //            plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new RenameTask(player), 1L);
        //        }
    }
    ...
    public void onPlayerKick(final PlayerKickEvent event) {
        ...
        if (plugin.changeKick()) {
            if (api.isNameChanged(player)) {
                event.setLeaveMessage(event.getLeaveMessage().replaceFirst(player.getName(), player.getDisplayName()));
            }
        }
    }
    ...
    public void onPlayerQuit(final PlayerQuitEvent event) {
        ...
        if (plugin.changeLogin()) {
            if (api.isNameChanged(player)) {
                event.setQuitMessage(event.getQuitMessage().replaceFirst(player.getName(), player.getDisplayName()));
            }
        }
    }
     ...
     * to the server.
     *
     * @param event The event that was triggered by sending a command.
     */
    @EventHandler(priority = EventPriority.LOWEST)
    public void onCommandPreprocess(PlayerCommandPreprocessEvent event) {
                ...
                switch (target.length) {
                    ...
                    case 1:
                        if (!target[0].getName().equalsIgnoreCase(args[1])) {
                            ...
                            sbCommand.append(args[0]).append(" ").append(target[0].getName());
        ...
        if (sbCommand != null) {
            ...
            log.info(DNCStrings.dnc_long + "Converted |" + event.getMessage() + "| to |" + sbCommand.toString() + "|.");
        }
    }
