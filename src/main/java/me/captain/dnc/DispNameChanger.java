...
public class DispNameChanger extends JavaPlugin {
    ...
    public DispNameChanger() {
        ...
        hCommands = new HashMap<>();

        Collator collator = Collator.getInstance(Locale.ENGLISH);

        collator.setStrength(Collator.PRIMARY);

        tmPlayers = new TreeMap<>(collator);
    ...
    public void onDisable() {
        log.info(DNCStrings.dnc_long + localization.getString(DNCStrings.INFO_DNC_DISABLED));
    }
    ...
    public void onEnable() {
        ...
        if (pTag != null) {
            ...
            log.info(DNCStrings.dnc_long + localization.getString(DNCStrings.INFO_TAGAPI));
        }
        else {
            bUseTagAPI = false;

            log.info(DNCStrings.dnc_long + localization.getString(DNCStrings.INFO_NO_TAGAPI));
        }

        log.info(DNCStrings.dnc_long + localization.getString(DNCStrings.INFO_DNC_COMMANDS));

        if (!isSaveOnQuit() && !isSaveOnRename()) {
            log.severe(DNCStrings.dnc_long + localization.getString(DNCStrings.INFO_SAVE_DISABLED));
        }

        log.info(DNCStrings.dnc_long + localization.getString(DNCStrings.INFO_DNC_ENABLED));
    }
    ...
    private void formatTranslations() {
        ...
        Object[] dncArgs = {getDescription().getVersion(), sb.toString(), getDescription().getName()};
        ...
        localization.setString(DNCStrings.INFO_TAGAPI, formatter.format(tagArgs));

        formatter.applyPattern(localization.getString(DNCStrings.INFO_DNC_ENABLED));

        localization.setString(DNCStrings.INFO_DNC_ENABLED, formatter.format(dncArgs));

        formatter.applyPattern(localization.getString(DNCStrings.INFO_DNC_DISABLED));

        localization.setString(DNCStrings.INFO_DNC_DISABLED, formatter.format(dncArgs));

        formatter.applyPattern(localization.getString(DNCStrings.INFO_DB_MAKE));

        localization.setString(DNCStrings.INFO_DB_MAKE, formatter.format(dncArgs));
    ...
    private void loadConfig() {
        File fConfig = new File(this.getDataFolder() + File.separator + "config.yml");

        File fProperties = new File(this.getDataFolder() + File.separator + "VERSION");
        ...
        if (!fProperties.exists()) {
            log.info(DNCStrings.dnc_long + "Unable to find Config Version info. Copying Defaults from Default Config.");
            ...
            if (pVersion.containsKey("VERSION")) {
                if (!pVersion.getProperty("VERSION").equals(this.getDescription().getVersion())) {
                    log.info(DNCStrings.dnc_long + "Different Config Version Detected: " + pVersion.getProperty("VERSION") + " Copying Defaults from Default Config.");
        ...
        if (usePrefixColor()) {
            sPrefixFull = ccPrefix + Character.toString(cPrefix) + ChatColor.RESET;
    ...
    private void setupDatabase() {
        try {
            ...
            log.info(DNCStrings.dnc_long + localization.getString(DNCStrings.INFO_DB_MAKE));
